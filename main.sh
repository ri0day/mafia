...
echo "finally ,nodes is ${node_list[*]}"
...
do	
	((client_idx++))
	file_exist_and_executable=$(ssh wumin@$client "[ -x "$node_hander_file" ]&&echo ok||echo fail")
	client_port_available=$(ssh wumin@$client "if /bin/netstat -nalt|grep 1080[3-4] 1>/dev/null 2>&1;then echo unavailabe;else echo availabe;fi" )
...
then
	...
	split -d -l $((${line%%/*}/$pices)) $1 $data_store_dir/${1%.*} 
fi
}
...
do
	ssh  wumin@${node_list[node_idx]} "sh /tmp/node.sh >/dev/null 2>&1 &"
done
